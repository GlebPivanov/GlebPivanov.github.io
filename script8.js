function popup(par) {
	...
	let bg = document.getElementById("popupbg");
	if (par === true) {
  ...
  let names = document.getElementById('InputName1');
  let email = document.getElementById('exampleInputEmail1');
  let msg = document.getElementById('exampleFormControlTextarea1');
  let cb = document.getElementById('exampleCheck1');
  function save() {
		...
		$.ajax({
		  ...
		  success: function (response) {
			if ((response.status == "success") && (cb.checked == true)) {
			  ...
			  alert("Ошибка. Согласитесь с политикой обработки персональных данных");
			}
		  }
		});
	  });
